...
from engines.symbol_memory import get_symbol_score
...
def evaluate_trade(
    ...
    """
    Evaluate a single trade with forecast, confidence, power tier, compounding logic,
    and symbol trust scoring.
    """
    data = market_data.get(symbol, {})
    confidence = calculate_confidence(symbol, data)
    forecast = predict_tp_sl(symbol, data)
    trust_score = get_symbol_score(symbol)

    if confidence < 0.95 or trust_score < 0.6:
        return {
            "symbol": symbol,
            "valid": False,
            "reason": "Low confidence or trust score",
